	...
	<body>
		...
		<form id="form_kirim_pesan" method="post">
			...
			<?php
				$query_penerima = mysqli_query($koneksi, "SELECT nik, nama_lengkap FROM tbl_user WHERE nik != $id_member");
				while ($daftar_penerima=mysqli_fetch_array($query_penerima)) {
			?>
				<option value="<?php echo $daftar_penerima['nik']; ?>"><?php echo $daftar_penerima['nama_lengkap']; ?></option>
			<?php } ?>
			</select>
			<br>
			Subyek Pesan
		...
		<table width="600" class="tabel_pesan" cellpadding="5" cellspacing="0">
			...
			<tbody>
	<?php
		$query_daftar_pesan = mysqli_query($koneksi, "SELECT P.*, M.nik, M.nama_lengkap FROM pesan P, tbl_user M WHERE P.id_pengirim=M.nik AND P.id_penerima='$id_member' ORDER BY P.id_pesan DESC");
